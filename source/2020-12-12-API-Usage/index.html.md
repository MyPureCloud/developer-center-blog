...
One of way for you to avoid these kinds of situations is to periodically monitor all of your OAuth clients using the Genesys Cloud ```Usage API```. Learning the "ins and out" of this 
...
* ```Usage API``` mechanics
* Retrieve total Genesys Cloud API calls broken out by OAuth 
* Retrieve total Genesys Cloud API calls grouped by OAuth client and the endpoints the client calls
...
submitted. Lets go ahead and walk through how to build a ```Usage API``` query.
...
## Retrieving total Genesys Cloud API calls broken out by OAuth
Lets start with the simplest type of query, asking the ```Usage API``` for a breakdown of all of the metrics for all API calls within the month of February 2020. We are going to use 
our [API Explorer](https://developer.mypurecloud.com/developer-tools/#/api-explorer) to invoke the ```Usage API``` endpoints. Our API Explorer tool lets you invoke all of the Genesys 
...
If you take the ```executionId``` returned above and now call the *GET /api/v2/usage/query/{executionId}/results* endpoint you will get back one of two results. If the 
query is long-running you will eventually get the results. For the above query your returned payload from the ```Usage API``` call would look something like this:
...
```json
{
  "results": [
    ...
    {
      "clientId": "846acb58-bf3e-4f01-afd2-ae00a848fddd",
    ...
    {
      "clientId": "739d79d6-d07f-42c1-ac51-5dc35ef2f834",
...
In the query above you can see that the ```Usage API``` return count metrics for the total number of requests along with the individual values for all 200x, 300x, 400x, 500x HTTP status codes. The ```Usage API``` also 
breaks HTTP 429 status codes into its own metric bucket because the HTTP status codes represent rate-limiting errors and are one of the most common errors are 
customers look for when putting examining API usage data. When only the the ```interval``` field is set in the query, the default grouping for the query will be at the 
OAuth client level. 
...
## Retrieving total Genesys Cloud API calls grouped by OAuth client and the endpoints the client calls
Remember, a single OAuth client can call multiple Genesys Cloud APIs. The ```Usage API``` allows you to breakdown the metrics returned into more fine-grained numbers via 
the ```groupBy``` field. To further refine our query to show endpoints called by each OAuth client lets add the ```groupBy``` field to the query:
...
```
Whoa! Now we get a lot more data back from our calls. That is because each entry returned from the above query represents the total count of HTTP status codes and requests 
by OAuth client and the individual Genesys Cloud APIs invoked by the client.
...
This field will allow you to break your numbers down into time buckets of ```Day```,```Week```, or ```Month```. Lets continue to build out our query using the ```Usage API``` to 
show all API usage for the month of Feb. 2020, broken out by OAuth Client and the URI called by the OAuth client. The query issued would 
look this:
...
Now that you have the data for your API usage extracted out of Genesys Cloud, you are going to often want to do some kind of analysis on the results. To keep things simple, 
...
Have a plan in place on how you are going to monitor the API usage before deploying an integration. Thinking about your integrations API usage ahead of time can prevent 
