...
tags: open source, https, secure, local, localhost, 127.0.0.1, bridge, connectors, action, service
date: 2018-09-11
author: patrick.barry@genesys.com
---

## The History Lesson
In the past, our customers have needed to rely on our Bridge Server if they wanted to call a service that was behind 
their firewall and they did not want to expose it externally (to the internet).  The Bridge Server was nice because it 
prevented companies from having to figure out how to safely expose internal-only services.  Once a Bridge Server was 
installed, you could then install any number of "Connectors" that would add the functionality you need.  One Connector, 
the Web Services Data Dip, would allow our customers to hit REST endpoints behind their firewall, and send the information 
...
![Purecloud Workflow](purecloudflow.png)
...
service accepts http traffic.
 ...
 1. Install ngrock https://dashboard.ngrok.com/get-started
    ...
    * You will need a token. Explained in (step 3 on page above)
    > When running ngrok in a non-dev environment, you will need to upgrade your ngrok license to either the Pro or 
...
2. The custom Action we are going to setup needs to belong to a PureCloud Integration. Lets create a test Integration..
    * Go to Integrations > Integrations
    ![Integrations](screenshot3.png)
    * Add a new Integration
        1. Install **Web Services Data Actions**
        ![Install Web Service Data Actions](screenshot4.png)
    ...
    * Create Custom Action
        ...
        ![Purecloud Actions](screenshot5.png)
        2. Click Import
        3. Import importExample.json
...
> Architecture Note: Would you use this approach to hit services that are publicly facing? No. In that scenario, you would 
still use a Custom Action in Purecloud, but instead of fronting it with the ngrok URL, you would just use the service's publicly facing URL.
