...
aws s3 sync s3://inin-static-assets/$S3_STATIC_ASSETBUCKET/$BUILD_NUMBER s3://inin-index-files-dev/developercenter-blog/blog --include "*" --acl "public-read" --cache-control "max-age=0, no-cache, no-store" --metadata-directive REPLACE --delete
